#!/usr/bin/env python3
"""
绘制两个数据集的向量误差 CDF 对比图（使用 meas - gt 向量误差）

用法:
    uv run python validators/adaptability/plot_vector_cdf.py \
        --dir1 results/model_resnet_0111_96_ABR-AL60 \
        --label1 "ABR-AL60" \
        --dir2 results/model_resnet_0111_96_Redmi-K30-Pro \
        --label2 "Redmi-K30-Pro"
...
参数:
    ...
    --dir2: 第二个数据目录路径
    --label2: 第二个数据集的标签
    --output: 输出文件路径，默认 results/compare/CDF_Comparison_Vector.png

输出:
    results/compare/CDF_Comparison_Vector.png
"""
...
def plot_comparison(
    ...
    ax.set_title(title if title else "Vector Error Comparison", fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(description="绘制两个数据集的向量误差CDF对比图")
    ...
    parser.add_argument("--output", type=str,
                        default="results/compare/CDF_Comparison_Vector.png",
                        help="输出文件路径")
    ...
    print("=== Vector CDF Comparison ===")
    datasets = []

    # 第一个数据集
    dir1 = Path(args.dir1)
    if dir1.exists():
        errors, stats = collect_vector_errors_from_result_dir(dir1)
        datasets.append({
            "name": args.label1,
            "errors": errors,
            "stats": stats,
        })
        print(f"  {args.label1}: {stats['trajectory_count']} trajectories, {stats['total_samples']} samples")
        print(f"    Mean: {stats['mean']:.4f}, Std: {stats['std']:.4f}")

    # 第二个数据集
    dir2 = Path(args.dir2)
    if dir2.exists():
        errors, stats = collect_vector_errors_from_result_dir(dir2)
        datasets.append({
            "name": args.label2,
            "errors": errors,
            "stats": stats,
        })
        print(f"  {args.label2}: {stats['trajectory_count']} trajectories, {stats['total_samples']} samples")
        print(f"    Mean: {stats['mean']:.4f}, Std: {stats['std']:.4f}")
    ...
    if len(datasets) >= 2:
        ...
        plot_comparison(
            datasets,
            output_path,
            title=f"{args.label1} vs {args.label2} - Vector Error",
        )
    else:
