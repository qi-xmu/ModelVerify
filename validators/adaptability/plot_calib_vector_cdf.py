#!/usr/bin/env python3
"""
绘制模型在相同设备下标定/非标定的CDF对比图（使用 meas - gt 向量误差）

用法:
    # 使用默认路径
    uv run python validators/adaptability/plot_calib_vector_cdf.py
...
输出:
    results/compare/CDF_CalibrationComparison_Vector.png
"""
...
def plot_calib_comparison(
    ...
    ax.set_title(title if title else "Calibration Comparison - Vector Error", fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(description="绘制标定/非标定的向量误差CDF对比图")
    ...
    print("=== Calibration CDF Comparison (Vector Error) ===")
    datasets = []
    ...
    if k30_calib_dir.exists():
        errors, stats = collect_vector_errors_from_result_dir(k30_calib_dir)
        datasets.append({
    ...
    if len(datasets) >= 2:
        output_path = output_dir / "CDF_CalibrationComparison_Vector.png"
        plot_calib_comparison(
            datasets,
            output_path,
            title="model_resnet_0111_96 - Calibration Comparison (Vector Error)",
        )
    else:
